...
DRV=$(echo $(grep -i -A 30 "Section.\+\"Device\"" /etc/X11/xorg.conf|grep -i "Driver"|grep "\".\+\""|sed s/"driver"/""/gi)|cut -f2 -d"\"");
...
then
  ...
  then
    ...
    PAKVER=$(dpkg -l $PAK|tail -1|sed s/" \+"/" "/g|cut -f2,3 -d" "|sed s/" "/" version "/g)
